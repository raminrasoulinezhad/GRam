...
- **A workout in progress now shows in History, and can always be found again.** It was always
  saved: every set is written to storage the instant you tick it, and always has been. What was
  missing was any way to see that. History listed finished workouts only, so closing the app
  mid-session and coming back to that tab showed no trace of the sets you had just recorded,
  which reads exactly like the app having thrown them away.
  ...
  so nothing sits marked IN PROGRESS for a week and its sets reach the log that counts them.

  A stale workout with nothing recorded in it is dropped, which is the line `endSession` already
  draws: everything in it is a target copied off a plan, never a result. No recorded set is
...
- **A feedback box in Profile.** There is no account here, no crash reporting and no analytics,
  ...
  and the card says so above the button. No email address appears anywhere in the app: the
  destination is configured server-side at the host, because anything compiled into the bundle
...
- **The exercise picker is a switch, not a tally.** Tapping a row twice used to add the exercise
  ...
  mis-tap in a list of nearly nine hundred near-identical rows could delete training that
  actually happened.
...
- **Glutes: the barbell hip thrust is no longer the first recommendation.** Found by the 1.9
  ...
  reverse lunge takes first place; the Romanian deadlift keeps second.
